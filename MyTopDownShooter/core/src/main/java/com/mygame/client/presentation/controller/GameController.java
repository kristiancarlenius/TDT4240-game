...
/**
 * Reads local player input each frame and forwards it to the server
 * at a fixed send rate (20 Hz). Does not mutate WorldState — that is
 * the server's responsibility via incoming snapshots.
 */
public final class GameController {
    ...
    /**
     * Called every render frame. Accumulates time and fires an input packet
     * each time SEND_DT is exceeded. Skips sending while the local player
     * is dead (respawn countdown).
     */
